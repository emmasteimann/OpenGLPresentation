...
//  GLuint _tessLevelInnerUniform;
//  GLuint _tessLevelOuterUniform;
}
...
//
//  GLuint tcsHandle = [self compileShader:@"GLTessControl.glsl"
//                                         withType:GL_TESS_CONTROL_SHADER];
//
//  GLuint tesHandle = [self compileShader:@"GLTessEval.glsl"
//                                         withType:GL_TESS_EVALUATION_SHADER];

  _programHandle = glCreateProgram();
  glAttachShader(_programHandle, vertexShaderName);
//  glAttachShader(_programHandle, tcsHandle);
//  glAttachShader(_programHandle, tesHandle);
  glAttachShader(_programHandle, fragmentShaderName);

  glBindAttribLocation(_programHandle, GLVertexAttribPosition, "a_Position");
  glBindAttribLocation(_programHandle, GLVertexAttribColor, "a_Color");
  glBindAttribLocation(_programHandle, GLVertexAttribTexCoord, "a_TexCoord");
  glBindAttribLocation(_programHandle, GLVertexAttribNormal, "a_Normal");
...
//  glUniform1f(_tessLevelInnerUniform, 3);
//  glUniform1f(_tessLevelOuterUniform, 2);
}
